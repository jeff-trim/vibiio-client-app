...
export class MyVibiiosComponent implements OnInit {
  ...
  scroll() {
    if (this.currentPage !== null) {
      this.myAppointmentsService
        .getMyAppointments(this.currentPage)
  ...
  getAppointments(pageNum: number, start_time?: number, end_time?: number) {
    this.myAppointmentsService
      .getMyAppointments(pageNum, start_time, end_time)
  ...
  addToAppointments(incoming_apts) {
    if (incoming_apts.length !== 0) {
      incoming_apts.map((appointment: Appointment) =>
  ...
  updateAppointment(apt_obj) {
    this.myDayService
      .updateMyDay(apt_obj.appointment.id, apt_obj.appointment.vibiiographer_id)
      .subscribe(response => {
  ...
  updateMySchedule() {
    this.myDayService.getMyDay().subscribe(response => {
