...
export class AppointmentNotificationComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.consumerName = JSON.parse(
      this.notificationData.content.message_body
    ).consumer;
    ...
    this.minutes =
      parseInt(
        JSON.parse(this.notificationData.content.message_body).minutes.replace(
          /^0+/,
          ""
        ),
        10
    ...
    this.seconds =
      parseInt(
        JSON.parse(this.notificationData.content.message_body).seconds.replace(
          /^0+/,
          ""
        ),
        10
